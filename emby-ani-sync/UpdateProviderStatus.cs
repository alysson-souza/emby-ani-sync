...
using emby_ani_sync.Models.Mal;
...
namespace emby_ani_sync
{
    public class UpdateProviderStatus
    {
        ...
        public async Task Update(BaseItem e, Guid userId, bool playedToCompletion)
        {
            ...
            if (
                LibraryCheck(_userConfig, _libraryManager, _logger, e)
                && video is Episode or Movie
                && playedToCompletion
            )
            {
                if (
                    (video is Episode && (episode.IndexNumber == null || episode.Season.IndexNumber == null))
                ...
                )
                {
                    ...
                    _apiIds = await AnimeOfflineDatabaseHelpers.GetProviderIdsFromMetadataProvider(
                        _httpClientFactory.CreateClient(),
                        _logger,
                        retrievedAniListId,
                        AnimeOfflineDatabaseHelpers.Source.Anilist
                    );
                    if (_apiIds is null)
                    {
                    ...
                    _animeType == typeof(Episode)
                        ...
                        : movie.ProviderIds != null && movie.ProviderIds.ContainsKey("Anidb")
                )
                {
                    ...
                    aniDbId =
                        _animeType == typeof(Episode)
                            ? await AnimeListHelpers.GetAniDbId(
                                _logger,
                                episode,
                                episode.IndexNumber.Value,
                                episode.Season.IndexNumber.Value,
                                animeListXml
                            )
                            : await AnimeListHelpers.GetAniDbId(
                                _logger,
                    ...
                    if (aniDbId.aniDbId != null)
                    {
                        _logger.LogInformation(
                            $"Retrieving provider IDs from offline database for AniDb ID {aniDbId.aniDbId.Value}..."
                        );
                        _apiIds = await AnimeOfflineDatabaseHelpers.GetProviderIdsFromMetadataProvider(
                            _httpClientFactory.CreateClient(),
                            _logger,
                            aniDbId.aniDbId.Value,
                            AnimeOfflineDatabaseHelpers.Source.Anidb
                        );
                        if (_apiIds is null)
                        {
                            _apiIds = new AnimeOfflineDatabaseHelpers.OfflineDatabaseResponse
                ...
                foreach (UserApiAuth userApiAuth in _userConfig.UserApiAuth)
                {
                    ...
                    {
                        case ApiName.Mal:
                            ApiCallHelpers = new ApiCallHelpers(
                                malApiCalls: new MalApiCalls(
                                    _httpClientFactory,
                                    _loggerFactory,
                            ...
                            if (
                                ...
                                && _apiIds.MyAnimeList != 0
                                && (episode != null && episode.Season.IndexNumber.Value != 0)
                            )
                            {
                                await CheckUserListAnimeStatus(
                                    _apiIds.MyAnimeList.Value,
                                    _animeType == typeof(Episode)
                                        ? (
                        ...
                        case ApiName.AniList:
                            ApiCallHelpers = new ApiCallHelpers(
                                aniListApiCalls: new AniListApiCalls(
                                    _httpClientFactory,
                                    _loggerFactory,
                                    _serverApplicationHost,
                                    _memoryCache,
                            ...
                            )
                            {
                                if (
                                    _animeType == typeof(Episode)
                                        ? int.TryParse(episode.Series.ProviderIds["AniList"], out int aniListId)
                                        : int.TryParse(movie.ProviderIds["AniList"], out aniListId)
                                )
                                {
                                    await CheckUserListAnimeStatus(
                                        aniListId,
                        ...
                        case ApiName.Shikimori:
                            ...
                            if (shikimoriAppName == null)
                                return;
                            ApiCallHelpers = new ApiCallHelpers(
                                shikimoriApiCalls: new ShikimoriApiCalls(
                                    _httpClientFactory,
                                    _loggerFactory,
                                    _serverApplicationHost,
                                    _memoryCache,
                        ...
                        case ApiName.Simkl:
                            ...
                            if (simklClientId == null)
                                return;
                            ApiCallHelpers = new ApiCallHelpers(
                                simklApiCalls: new SimklApiCalls(
                                    _httpClientFactory,
                                    _loggerFactory,
                                    _serverApplicationHost,
                                    _memoryCache,
                            ...
                            if (
                                (
                                    (_apiIds.MyAnimeList != null && _apiIds.MyAnimeList != 0)
                            ...
                            )
                            {
                                await CheckUserListAnimeStatus(
                                    ...
                                    _animeType == typeof(Episode)
                                        ...
                                        : movie.IndexNumber.Value,
                                    _animeType == typeof(Episode) ? episode.SeriesName : video.Name,
                                    false
                                );
                                continue;
                            }
                    ...
                    List<Anime> animeList = await ApiCallHelpers.SearchAnime(
                        _animeType == typeof(Episode) ? episode.SeriesName : video.Name
                    );
                    bool found = false;
                    if (animeList != null)
                    {
                        foreach (var anime in animeList)
                        {
                            ...
                            if (
                                checkMalId && _apiIds.MyAnimeList != null && anime.Id == _apiIds.MyAnimeList
                                || !checkMalId && TitleCheck(anime, episode, movie)
                            )
                            {
                                _logger.LogInformation(
                                    $"({ApiName}) Found matching {animeType}: {GetAnimeTitle(anime)}"
                                );
                                ...
                                if (_animeType == typeof(Episode))
                                {
                                    ...
                                    if (!checkMalId)
                                    {
                                        ...
                                        if (episode?.Season.IndexNumber is > 1)
                                        {
                                            ...
                                            matchingAnime = await GetDifferentSeasonAnime(
                                                anime.Id,
                                                episode.Season.IndexNumber.Value,
                                            ...
                                            _logger.LogInformation(
                                                $"({ApiName}) Season being watched is {GetAnimeTitle(matchingAnime)}"
                                            );
                                        }
                                        else if (episode?.Season.IndexNumber == 0)
                                        {
                                            // the episode is an ova or special
                                            matchingAnime = await GetOva(
                                                anime.Id,
                                                episode.Name,
                                                alternativeId: anime.AlternativeId
                                            );
                                        ...
                                        {
                                            _logger.LogInformation(
                                                $"({ApiName}) Watched episode passes total episodes in season! Checking for additional seasons/cours..."
                                            );
                                            // either we have found the wrong series (highly unlikely) or it is a multi cour series/Emby has grouped next season into the current.
                                            ...
                                            {
                                                var nextSeason = await GetDifferentSeasonAnime(
                                                    season.Id,
                                                    seasonCounter + 1,
                                                    alternativeId: season.AlternativeId
                                                );
                                                if (nextSeason == null)
                                                {
                                                    _logger.LogWarning($"({ApiName}) Could not find next season");
                                                    if (
                                                        matchingAnime.Status == AiringStatus.currently_airing
                                                        && matchingAnime.NumEpisodes == 0
                                                    )
                                                    {
                                                        _logger.LogWarning(
                                                            $"({ApiName}) Show is currently airing and API reports 0 episodes, going to use first season"
                                                        );
                                                        isRootSeason = true;
                                                    }
                                                ...
                                                await CheckUserListAnimeStatus(
                                                    season.Id,
                                                    season.NumEpisodes,
                                                    overrideCheckRewatch: false,
                                                    alternativeId: matchingAnime.AlternativeId
                                                );
                                                season = nextSeason;
                                            }
        ...
        /// <summary>
        /// Checks if the Emby library entry matches the API names.
        /// </summary>
        /// <param name="anime">The API anime.</param>
        ...
        private bool TitleCheck(Anime anime, Episode episode, Movie movie)
        {
            ...
            return CompareStrings(anime.Title, title)
                ...
                || (
                    anime.AlternativeTitles.Synonyms != null
                    && anime.AlternativeTitles.Synonyms.Any(synonym => CompareStrings(synonym, title))
                );
        }
        ...
        private bool ContainsExtended(string first, string second)
        {
            return StringFormatter
                    .RemoveSpecialCharacters(first)
                    .IndexOf(StringFormatter.RemoveSpecialCharacters(second), StringComparison.OrdinalIgnoreCase) >= 0;
        }
        ...
        /// <returns></returns>
        public static bool LibraryCheck(
        ...
        private async Task CheckUserListAnimeStatus(
            AnimeOfflineDatabaseHelpers.OfflineDatabaseResponse matchingIds,
            int episodeNumber,
            string title,
            bool overrideCheckRewatch,
        ...
        private async Task CheckUserListAnimeStatusBase(
            Anime detectedAnime,
            int episodeNumber,
            bool overrideCheckRewatch,
            string? alternativeId = null
        )
        {
            if (detectedAnime == null)
                return;
            if (
                detectedAnime.MyListStatus != null
                && detectedAnime.MyListStatus.Status == Status.Watching
                && ApiName != ApiName.Annict
            )
            {
                _logger.LogInformation(
                    $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) found on watching list"
                );
            ...
            if (_userConfig.PlanToWatchOnly)
            {
                ...
                {
                    _logger.LogInformation(
                        $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) found on plan to watch list"
                    );
                ...
                if (ApiName != ApiName.Annict)
                {
                    ...
                    _logger.LogInformation(
                        $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) not found in plan to watch list{(_userConfig.RewatchCompleted ? ", checking completed list.." : null)}"
                    );
                    updated = true;
            ...
            if (detectedAnime.MyListStatus != null)
            {
                ...
                _logger.LogInformation(
                    $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) found on {detectedAnime.MyListStatus.Status} list"
                );
                if (detectedAnime.MyListStatus.Status == Status.Completed)
                {
                    if (ApiName != ApiName.Annict)
                        _logger.LogInformation(
                            $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) found on Completed list, but user does not want to automatically set as rewatching. Skipping"
                        );
                    return;
                }
            }
            ...
            {
                _logger.LogInformation(
                    $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) not on user list"
                );
            }
        ...
        private async Task UpdateAnnictStatus(Anime detectedAnime, int episodeNumber)
        {
            ...
            {
                _logger.LogInformation(
                    $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) found, but not on plan to watch list"
                );
                return;
            }
            ...
            {
                _logger.LogInformation(
                    $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) complete, marking anime as complete"
                );
            ...
            {
                _logger.LogInformation(
                    $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) being marked as watching"
                );
                await ApiCallHelpers.UpdateAnime(
                    detectedAnime.Id,
                    1,
            ...
            {
                _logger.LogInformation(
                    $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) already set to watching, not updating again"
                );
            }
        }

        private async Task<bool> CheckIfRewatchCompleted(
            Anime detectedAnime,
            int indexNumber,
            bool overrideCheckRewatch
        )
        {
            if (
                overrideCheckRewatch
                || detectedAnime.MyListStatus is { Status: Status.Completed }
                || detectedAnime.MyListStatus is { Status: Status.Rewatching }
                    && detectedAnime.MyListStatus.NumEpisodesWatched < indexNumber
            )
            {
                if (ApiName == ApiName.Simkl)
                {
                    _logger.LogInformation(
                        $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) found on completed list, but {ApiName} does not support re-watching. Skipping"
                    );
                    return true;
                }
                if (_userConfig.RewatchCompleted)
                {
                    ...
                    {
                        _logger.LogInformation(
                            $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) found on completed list, setting as re-watching"
                        );
                        await UpdateAnimeStatus(
                            detectedAnime,
                            indexNumber,
                            true,
                            detectedAnime.MyListStatus.RewatchCount,
                            true
                        );
                        return true;
                    }
            ...
            {
                _logger.LogInformation(
                    $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) found, but provider reports episode already watched. Skipping"
                );
                return true;
            }
            ...
            {
                _logger.LogInformation(
                    $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) found, but not on completed or plan to watch list. Skipping"
                );
                return true;
            }
        ...
        private async Task<Anime> GetAnime(int animeId, Status? status = null, string? alternativeId = null)
        {
            ...
            if (
                anime != null
                && (
                    (status != null && anime.MyListStatus != null && anime.MyListStatus.Status == status)
                    || status == null
                )
            )
            {
        ...
        private async Task<Anime> GetAnime(
            AnimeOfflineDatabaseHelpers.OfflineDatabaseResponse animeIds,
            string title,
            Status? status = null,
            string? alternativeId = null
        )
        ...
        internal async Task UpdateAnimeStatus(
            Anime detectedAnime,
            int? episodeNumber,
            bool? setRewatching = null,
            int? rewatchCount = null,
            bool firstTimeRewatch = false
        )
        {
            if (episodeNumber != null)
            {
                ...
                {
                    if (
                        detectedAnime.MyListStatus.NumEpisodesWatched < episodeNumber.Value
                        || detectedAnime.NumEpisodes == 1
                        || (
                            setRewatching != null
                            && setRewatching.Value
                            && detectedAnime.MyListStatus.NumEpisodesWatched == episodeNumber.Value
                    ...
                    )
                    {
                        ...
                        if (episodeNumber.Value == detectedAnime.NumEpisodes || detectedAnime.NumEpisodes == 1)
                        {
                            ...
                            if (detectedAnime.NumEpisodes == 1)
                            {
                                ...
                                response = await ApiCallHelpers.UpdateAnime(
                                    detectedAnime.Id,
                                    1,
                                    Status.Completed,
                                    startDate: detectedAnime.MyListStatus.IsRewatching
                                    || detectedAnime.MyListStatus.Status == Status.Completed
                                        ? null
                                        : DateTime.Now,
                                    endDate: detectedAnime.MyListStatus.IsRewatching
                                    || detectedAnime.MyListStatus.Status == Status.Completed
                                        ? null
                                        : DateTime.Now,
                                    isRewatching: false,
                                    numberOfTimesRewatched: (setRewatching != null && setRewatching.Value)
                                    || detectedAnime.MyListStatus.IsRewatching
                            ...
                            _logger.LogInformation(
                                $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) complete, marking anime as complete{(ApiName != ApiName.Mal && (setRewatching != null && setRewatching.Value) ? ", increasing re-watch count by 1" : "")}"
                            );
                            if (
                                (
                                    detectedAnime.MyListStatus.IsRewatching
                                    ...
                                    || (setRewatching != null && setRewatching.Value)
                                )
                                && ApiName == ApiName.Mal
                            )
                            {
                                ...
                                _logger.LogInformation(
                                    $"({ApiName}) {(_animeType == typeof(Episode) ? "Series" : "Movie")} ({GetAnimeTitle(detectedAnime)}) has also been re-watched, increasing re-watch count by 1"
                                );
                                response = await ApiCallHelpers.UpdateAnime(
                                    detectedAnime.Id,
                                    episodeNumber.Value,
                        ...
                        else
                        {
                            if (detectedAnime.MyListStatus.IsRewatching && ApiName == ApiName.Mal)
                            {
                                ...
                                _logger.LogInformation(
                                    $"({ApiName}) User is re-watching {(_animeType == typeof(Episode) ? "series" : "movie")} ({GetAnimeTitle(detectedAnime)}), set as completed but update re-watch progress"
                                );
                                response = await ApiCallHelpers.UpdateAnime(
                                    detectedAnime.Id,
                                    episodeNumber.Value,
                            ...
                            else
                            {
                                if (episodeNumber > 1)
                                {
                                    ...
                                    response = await ApiCallHelpers.UpdateAnime(
                                        detectedAnime.Id,
                                        episodeNumber.Value,
                                        detectedAnime.MyListStatus.IsRewatching && ApiName == ApiName.AniList
                                            ? Status.Rewatching
                                            : Status.Watching,
                                        alternativeId: detectedAnime.AlternativeId,
                                        ids: _apiIds,
                                ...
                                {
                                    _logger.LogInformation(
                                        $"({ApiName}) Setting new {(_animeType == typeof(Episode) ? "series" : "movie")} ({GetAnimeTitle(detectedAnime)}) as watching."
                                    );
                                    response = await ApiCallHelpers.UpdateAnime(
                                        detectedAnime.Id,
                                        episodeNumber.Value,
                        ...
                        {
                            _logger.LogInformation(
                                $"({ApiName}) Updated {(_animeType == typeof(Episode) ? "series" : "movie")} ({GetAnimeTitle(detectedAnime)}) progress to {episodeNumber.Value}"
                            );
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            _logger.LogInformation(
                                $"({ApiName}) Series ({GetAnimeTitle(detectedAnime)}) has already been watched, marking anime as re-watching; progress of {episodeNumber.Value}"
                            );
                        ...
                        else
                        {
                            response = null;
                            _logger.LogInformation(
                                $"({ApiName}) Provider reports episode already watched; not updating"
                            );
                        }
                    }
                }
                else
                {
                    ...
                    if (episodeNumber.Value == detectedAnime.NumEpisodes)
                    {
                        ...
                        _logger.LogInformation(
                            $"({ApiName}) Adding new {(_animeType == typeof(Episode) ? "series" : "movie")} ({GetAnimeTitle(detectedAnime)}) to user list as completed with a progress of {episodeNumber.Value}"
                        );
                        response = await ApiCallHelpers.UpdateAnime(
                            detectedAnime.Id,
                            episodeNumber.Value,
                    ...
                    else
                    {
                        ...
                        _logger.LogInformation(
                            $"({ApiName}) Adding new {(_animeType == typeof(Episode) ? "series" : "movie")} ({GetAnimeTitle(detectedAnime)}) to user list as watching with a progress of {episodeNumber.Value}"
                        );
                        response = await ApiCallHelpers.UpdateAnime(
                            detectedAnime.Id,
                            episodeNumber.Value,
        ...
        /// <summary>
        /// Get further anime seasons. Emby uses numbered seasons whereas MAL uses entirely different entities.
        /// </summary>
        ...
        internal async Task<Anime?> GetDifferentSeasonAnime(int animeId, int seasonNumber, string? alternativeId = null)
        {
            ...
            Anime retrievedSeason = await ApiCallHelpers.GetAnime(
                animeId,
                getRelated: true,
            ...
            if (retrievedSeason != null)
            {
                ...
                {
                    RelatedAnime? initialSeasonRelatedAnime = retrievedSeason.RelatedAnime?.FirstOrDefault(item =>
                        item.RelationType == RelationType.Sequel
                    );
                    if (initialSeasonRelatedAnime != null)
                    {
                        _logger.LogInformation($"({ApiName}) Attempting to get season {i + 1}...");
                        Anime nextSeason = await ApiCallHelpers.GetAnime(
                            initialSeasonRelatedAnime.Anime.Id,
                            getRelated: true,
                            alternativeId: initialSeasonRelatedAnime.Anime.AlternativeId
                        );
        ...
        private async Task<Anime> GetOva(int animeId, string episodeName, string? alternativeId = null)
        {
            ...
            if (anime != null)
            {
                var listOfRelatedAnime = anime.RelatedAnime.Where(relation =>
                    relation.RelationType
                        is RelationType.Side_Story
                            or RelationType.Alternative_Version
                            or RelationType.Alternative_Setting
                );
                foreach (RelatedAnime relatedAnime in listOfRelatedAnime)
                {
                    var detailedRelatedAnime = await ApiCallHelpers.GetAnime(
                        relatedAnime.Anime.Id,
                        alternativeId: relatedAnime.Anime.AlternativeId
                    );
                    if (detailedRelatedAnime is { Title: { }, AlternativeTitles: { En: { } } })
                    {
                        if (
                            ContainsExtended(detailedRelatedAnime.Title, episodeName)
                            || (
                                detailedRelatedAnime.AlternativeTitles.En != null
                                && ContainsExtended(detailedRelatedAnime.AlternativeTitles.En, episodeName)
                            )
                            || (
                                detailedRelatedAnime.AlternativeTitles.Ja != null
                                && ContainsExtended(detailedRelatedAnime.AlternativeTitles.Ja, episodeName)
                            )
                        )
                        {
                            // rough match
