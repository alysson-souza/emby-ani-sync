...
public class ConfigHelper
{
    ...
    {
        string? simklClientId = Plugin
            .Instance?.PluginConfiguration.ProviderApiAuth?.FirstOrDefault(item => item.Name == ApiName.Simkl)
            ?.ClientId;
    ...
    public static (bool success, string? reason) DeauthenticateUser(Guid userId, ApiName provider)
    {
        if (Plugin.Instance == null)
            return (false, "Plugin instance null.");
        UserConfig? userConfig = Plugin.Instance.PluginConfiguration.UserConfig.FirstOrDefault(userConfig =>
