...
namespace emby_ani_sync.Helpers
{
    public class GraphQlHelper
    {
        ...
        {
            var call = await httpClient.PostAsync(
                "https://graphql.anilist.co",
                new StringContent(
                    JsonSerializer.Serialize(new GraphQl { Query = query, Variables = variables }),
                    Encoding.UTF8,
                    "application/json"
                )
            );
        ...
        {
            AuthApiCall authApiCall = new AuthApiCall(
                httpClientFactory,
            ...
            var call = await authApiCall.AuthenticatedApiCall(
                provider,
        ...
        public static async Task<T> DeserializeRequest<T>(
            HttpClient httpClient,
            string query,
