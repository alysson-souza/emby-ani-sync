...
namespace emby_ani_sync.Helpers
{
    ...
    {
        public static async Task<OfflineDatabaseResponse> GetProviderIdsFromMetadataProvider(
        ...
        )
        {
            ...
            {
                if (
                    Uri.TryCreate(customArmServerBaseUrl, UriKind.Absolute, out Uri baseUri)
                ...
                {
                    logger.LogWarning(
                        $"ARM server base URL ({customArmServerBaseUrl}) could not be parsed. Confirm the URL is valid. Falling back to public API..."
                    );
                }
            }
            ...
            try
            {
                response = await httpClient.GetAsync(
                    $"{baseUrl}/api/v2/ids?source={source.ToString().ToLower()}&id={metadataId}"
                );
            }
            catch (Exception e)
            {
                logger.LogError(
                    "Error encountered while retrieving provider IDs: {Message}\n{StackTrace}",
                    e.Message,
                    e.StackTrace
                );
                return null;
