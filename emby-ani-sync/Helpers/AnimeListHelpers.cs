...
namespace emby_ani_sync.Helpers
{
    public class AnimeListHelpers
    {
        ...
        /// <returns></returns>
        public static async Task<(int? aniDbId, int? episodeOffset)> GetAniDbId(
            ILogger logger,
            Video video,
            int episodeNumber,
            int seasonNumber,
            AnimeListXml animeListXml
        )
        {
            int aniDbId;
            if (animeListXml == null)
                return (null, null);
            Dictionary<string, string> providers;
            if (video is Episode)
            {
                //Search for Anidb id at season level
                providers = (video as Episode).Season.ProviderIds.ContainsKey("Anidb")
                    ? (video as Episode).Season.ProviderIds
                    : (video as Episode).Series.ProviderIds;
            }
            ...
            if (providers.ContainsKey("Anidb"))
            {
                ...
                if (!int.TryParse(providers["Anidb"], out aniDbId))
                    return (null, null);
                var foundAnime = animeListXml
                    .Anime.Where(anime =>
                        int.TryParse(anime.Anidbid, out int xmlAniDbId)
                        && xmlAniDbId == aniDbId
                        && (
                            (video as Episode).Season.ProviderIds.ContainsKey("Anidb")
                            || (
                                int.TryParse(anime.Defaulttvdbseason, out int xmlSeason) && xmlSeason == seasonNumber
                ...
                {
                    case 1:
                        var related = animeListXml
                            .Anime.Where(anime => anime.Tvdbid == foundAnime.First().Tvdbid)
                            .ToList();
                        if (
                            video is Episode episode
                            && episode.Series.GetChildren(new InternalItemsQuery()).OfType<Season>().Count() > 1
                            && related.Count > 1
                        )
                        {
                            ...
                            logger.LogInformation(
                                $"(Anidb) Looking up anime {episode.Series.Name} in the anime XML file by absolute episode number..."
                            );
                            var (aniDb, episodeOffset) = GetAniDbByEpisodeOffset(
                                logger,
                                GetAbsoluteEpisodeNumber(episode),
                                seasonNumber,
                                episodeNumber,
                                related
                            );
                            ...
                            {
                                logger.LogInformation(
                                    $"(Anidb) Anime {episode.Series.Name} found in anime XML file, detected AniDB ID {aniDb}"
                                );
                            ...
                            {
                                logger.LogInformation(
                                    $"(Anidb) Anime {episode.Series.Name} could not found in anime XML file; falling back to other metadata providers if available..."
                                );
                            }
                        }
                        else
                        {
                            if (
                                video is Episode episodeWithMultipleSeasons
                                && episodeWithMultipleSeasons.Season.IndexNumber > 1
                            )
                            {
                                // user doesnt have full series; have to do season lookup
                                logger.LogInformation(
                                    $"(Anidb) Anime {episodeWithMultipleSeasons.Series.Name} found in anime XML file"
                                );
                            ...
                            else
                            {
                                ...
                                return int.TryParse(related.First().Anidbid, out aniDbId)
                                    ? (aniDbId, null)
                                    : (null, null);
                            }
                            logger.LogInformation(
                                $"(Anidb) Anime {(video is Episode episodeWithoutSeason ? episodeWithoutSeason.Name : video.Name)} found in anime XML file"
                            );
                            // is movie / only has one season / no related; just return the only result
                            return int.TryParse(foundAnime.First().Anidbid, out aniDbId)
                                ? (aniDbId, null)
                                : (null, null);
                        }
                    ...
                    case > 1:
                        // here
                        logger.LogWarning(
                            "(Anidb) More than one result found; possibly an issue with the XML. Falling back to other metadata providers if available..."
                        );
                        break;
                    case 0:
                        logger.LogWarning(
                            "(Anidb) Anime not found in anime list XML; falling back to other metadata providers if available..."
                        );
                        break;
                }
            }
            ...
            {
                int tvDbId;
                if (!int.TryParse(providers["Tvdb"], out tvDbId))
                    return (null, null);
                var related = animeListXml
                    .Anime.Where(anime => int.TryParse(anime.Tvdbid, out int xmlTvDbId) && xmlTvDbId == tvDbId)
                    .ToList();

                if (!related.Any())
                {
                    logger.LogWarning(
                        "(Tvdb) Anime not found in anime list XML; querying the appropriate providers API"
                    );
                    return (null, null);
                }
                ...
                )
                {
                    ...
                    {
                        logger.LogInformation(
                            $"(Tvdb) Anime {episode.Series.Name} found in anime XML file, detected AniDB ID {aniDb}"
                        );
                    ...
                    {
                        logger.LogInformation(
                            $"(Tvdb) Anime {episode.Series.Name} could not found in anime XML file; falling back to other metadata providers if available..."
                        );
                    }
                }
                else
                {
                    ...
                    )
                    {
                        // user doesnt have full series; have to do season lookup
                        logger.LogInformation(
                            $"(Tvdb) Anime {episodeWithMultipleSeasons.Name} found in anime XML file"
                        );
                        return SeasonLookup(logger, seasonNumber, episodeNumber, related);
                    }
                    else
        ...
        private static (int? aniDbId, int? episodeOffset) GetAniDbByEpisodeOffset(
            ILogger logger,
            int? absoluteEpisodeNumber,
            int seasonNumber,
            int episodeNumber,
            List<AnimeListAnime> related
        )
        {
            if (absoluteEpisodeNumber != null)
            {
                ...
                // numbers.
                var foundMapping = related.FirstOrDefault(animeListAnime =>
                    animeListAnime.MappingList?.Mapping?.FirstOrDefault(mapping =>
                        mapping.Start < absoluteEpisodeNumber && mapping.End > absoluteEpisodeNumber
                    ) != null
                );
                ...
                {
                    logger.LogWarning(
                        "(AniDb) Could not lookup using absolute episode number (reason: no mappings found)"
                    );
                    return SeasonLookup(logger, seasonNumber, episodeNumber, related);
                }
            }
            ...
            {
                logger.LogWarning(
                    "(AniDb) Could not lookup using absolute episode number (reason: absolute episode number is null)"
                );
                return SeasonLookup(logger, seasonNumber, episodeNumber, related);
            }
        }

        private static (int? aniDbId, int? episodeOffset) SeasonLookup(
            ILogger logger,
            int seasonNumber,
            int episodeNumber,
            List<AnimeListAnime> related
        ...
        {
            var previousSeasons = episode
                .Series.GetChildren(new InternalItemsQuery())
                .OfType<Season>()
                .Where(item => item.IndexNumber < episode.Season.IndexNumber)
                .ToList();
            int previousSeasonIndexNumber = -1;
            foreach (
                int indexNumber in previousSeasons
                    .Where(item => item.IndexNumber != null)
                    .Select(item => item.IndexNumber)
            ...
            var previousSeasonsEpisodeCount = previousSeasons
                .SelectMany(item => item.GetChildren(new InternalItemsQuery()).OfType<Episode>())
                .Count();
        ...
        public static AnimeListAnime GetAniDbSeason(int aniDbId, AnimeListXml animeListXml)
        {
            ...
            return animeListXml.Anime.FirstOrDefault(anime =>
                int.TryParse(anime.Anidbid, out int xmlAniDbId) && xmlAniDbId == aniDbId
            );
        }
        ...
        public static IEnumerable<AnimeListAnime> ListAllSeasonOfAniDbSeries(int aniDbId, AnimeListXml animeListXml)
        {
            ...
            AnimeListAnime foundXmlAnime = animeListXml.Anime.FirstOrDefault(anime =>
                int.TryParse(anime.Anidbid, out int xmlAniDbId) && xmlAniDbId == aniDbId
            );
            if (foundXmlAnime == null)
                return null;
        ...
        /// <param name="logger">Logger.</param>
        /// <returns></returns>
        public static async Task<AnimeListXml> GetAnimeListFileContents(
            ILogger logger,
        ...
        )
        {
            ...
            {
                logger.LogError(
                    $"Could not deserialize anime list XML; {e.Message}. Try forcibly redownloading the XML file"
                );
                return null;
            }
        }
        ...
        public class AnimeListXml
        {
            ...
            public List<AnimeListAnime> Anime { get; set; }
        }
    }
}
