...
public class TaskProcessMarkedMedia
{
    ...
    public async Task RunUpdate()
    {
        ...
        while (itemsToUpdate.Any())
        {
            ...
            var aniSyncConfigUser = Plugin.Instance?.PluginConfiguration.UserConfig.FirstOrDefault(uc =>
                uc.UserId == item.userId
            );
            List<(Guid userId, Guid? seasonId, Video baseItem)> pairedItems =
                new List<(Guid userId, Guid? seasonId, Video baseItem)>();
            if (
                aniSyncConfigUser != null
                && UpdateProviderStatus.LibraryCheck(aniSyncConfigUser, _libraryManager, _logger, item.baseItem)
            )
            {
                ...
                pairedItems = itemsToUpdate
                    .Where(items => items.seasonId == item.seasonId && items.userId == item.userId)
                    .ToList();
                item = pairedItems.OrderByDescending(i => i.baseItem.IndexNumber).FirstOrDefault();
