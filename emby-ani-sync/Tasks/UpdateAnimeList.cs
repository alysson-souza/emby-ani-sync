...
namespace emby_ani_sync
{
    public class UpdateAnimeList
    {
        ...
        )
        {
            ...
            else
            {
                Path = System.IO.Path.Combine(
                    Plugin.Instance.PluginConfiguration.animeListSaveLocation,
                    "anime-list-full.xml"
                );
            }
        ...
        public async Task<bool> Update()
        {
            ...
            client.DefaultRequestHeaders.UserAgent.ParseAdd("emby-ani-sync/1.0");
            var response = await client.GetAsync(
                "https://api.github.com/repos/Anime-Lists/anime-lists/contents/anime-list-full.xml"
            );
        ...
        private async Task<bool> CheckIfFileSizeHasChanged(int fileSize)
        {
            ...
            if (animeListFile.Exists)
                return animeListFile.Length != fileSize;
            return true;
        }
        ...
        private async Task GetLatestAnimeList(HttpClient httpClient, string downloadUrl)
        {
            try
            {
                Directory.CreateDirectory(
                    Path.EndsWith("anime-list-full.xml") ? Path.Replace("anime-list-full.xml", "") : Path
                );
