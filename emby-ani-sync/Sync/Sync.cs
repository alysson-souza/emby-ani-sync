...
public class Sync
{
    ...
    private readonly SyncJobTracker _syncJobTracker;
    ...
    )
    {
        ...
        _apiName = apiName;
        _status = status;
        _syncJobTracker = syncJobTracker;
        _jobId = jobId;
        _logger = loggerFactory.CreateLogger<Sync>();
    }

    /// <summary>
    /// Sync Emby with the selected providers watch list.
    /// </summary>
    ...
    {
        ReportProgress("Fetching provider list", $"Loading the {_apiName} watch list for the selected user.");
        var completedList = await GetAnimeList(userId);
        if (completedList == null || completedList.Count == 0)
        {
            string message =
                "No anime were found for this provider. Make sure the user is connected and the provider list is populated.";
            ReportProgress("Nothing to sync", message);
            _logger.LogWarning(
                "(Sync) No anime found by provider; please make sure user is authenticated with this provider and the authenticated users watch list is populated"
            );
            return;
        }

        ReportProgress(
            "Resolving anime metadata",
            "Matching provider entries to Emby metadata.",
            0,
            completedList.Count
        );
        ...
        {
            ReportProgress("Nothing matched", "Ani-Sync could not match any provider entries to Emby metadata.");
            return;
        }

        ReportProgress(
            "Updating Emby library",
            "Applying provider progress to the Emby library.",
            0,
            metadataIds.Count
        );
        await GetCurrentLibrary(userId, metadataIds);
        ReportProgress("Finishing", "Finalizing Emby updates.");
    }
            ...
            case ApiName.Mal:
                apiCallHelpers = new ApiCallHelpers(
                    malApiCalls: new MalApiCalls(
                        ...
                        _memoryCache,
                        _delayer,
                        Plugin.Instance.PluginConfiguration.UserConfig.FirstOrDefault(item =>
    ...
    {
        var userLibrary = SyncHelper.GetUsersLibrary(Guid.Parse(userId), _userManager, _libraryManager);
        ...
        for (int seriesIndex = 0; seriesIndex < seriesList.Count; seriesIndex++)
        {
            ...
            {
                List<(Season, DateTime?, int)> seasonsToMarkAsPlayed = await SyncHelper.GetLocalSeasons(
                    convertedWatchList,
                    providerId.Value,
                    series,
                    source.Value,
                ...
                foreach (
                    (Season season, DateTime? completedAt, int episodesWatched) seasonsTuple in seasonsToMarkAsPlayed
                )
                {
                    if (seasonsTuple.season != null)
                    {
                        var seasonEpisodes = seasonsTuple
                            .season.GetChildren(new InternalItemsQuery())
                            .Where(episode => episode is Episode && episode.IndexNumber != null);
                        if (seasonsTuple.episodesWatched != -1)
                        {
                            seasonEpisodes = seasonEpisodes.Where(episode =>
                                episode.IndexNumber != null && episode.IndexNumber <= seasonsTuple.episodesWatched
                            );
                        }
                        ...
                        foreach (var seasonChild in seasonEpisodes)
                        {
                            if (seasonChild is Episode episode)
                            {
                                _logger.LogInformation(
                                    $"(Sync) Setting {episode.Series.Name} season {episode.Season.IndexNumber} episode {episode.IndexNumber} for user {userId} as played..."
                                );
                                ...
                                if (seasonsTuple.completedAt != null)
                                {
                                    _userDataManager.SaveUserData(
                                        user,
                                        seasonChild,
                                        SetUserData(user, episode, seasonsTuple.completedAt),
                                ...
                                else
                                {
                                    _userDataManager.SaveUserData(
                                        user,
                                        seasonChild,
                                        SetUserData(user, episode, DateTime.UtcNow),
                                        UserDataSaveReason.TogglePlayed,
                                        CancellationToken.None
                                    );
                                }
                        ...
                        _userDataManager.SaveUserData(
                            user,
                            seasonsTuple.season,
                            SetUserData(user, seasonsTuple.season, seasonsTuple.completedAt),
                            UserDataSaveReason.TogglePlayed,
                            CancellationToken.None
                        );
    ...
    private async Task<List<SyncAnimeMetadata>> GetMetadataIdsFromAnime(List<Anime> animeList)
    {
        ...
        for (var i = 0; i < animeList.Count; i++)
        {
            ReportProgress(
                "Resolving anime metadata",
                $"Matching provider entry {i + 1} of {animeList.Count}.",
                i + 1,
                animeList.Count
            );
            _logger.LogInformation($"(Sync) Fetching IDs for anime with an ID of {animeList[i].Id}...");
            var ids = await AnimeOfflineDatabaseHelpers.GetProviderIdsFromMetadataProvider(
                _httpClientFactory.CreateClient(),
                _logger,
                animeList[i].Id,
                AnimeOfflineDatabaseHelpers.MapFromApiName(_apiName)
            );
            if (ids?.AniDb == null)
            {
            ...
            if (
                season == null
                || (!int.TryParse(season.Defaulttvdbseason, out var seasonNumber) && season.Defaulttvdbseason != "a")
            )
            {
            ...
            {
                episodesWatched = int.TryParse(season.Episodeoffset, out int offset)
                    ? animeList[i].MyListStatus.NumEpisodesWatched + offset
                    : animeList[i].MyListStatus.NumEpisodesWatched;
            }
            else
            {
                episodesWatched = -1;
