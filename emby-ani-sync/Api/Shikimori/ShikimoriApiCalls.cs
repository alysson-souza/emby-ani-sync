...
namespace emby_ani_sync.Api.Shikimori;
...
public class ShikimoriApiCalls
{
    ...
    {
        UrlBuilder url = new UrlBuilder { Base = $"{_apiBaseUrl}/users/whoami" };
        ...
        if (apiCall != null)
        {
            ...
            try
            {
                ...
                if (user != null)
                {
                    _userConfig?.KeyPairs.Add(new KeyPairs { Key = "ShikimoriUserId", Value = user.Id.ToString() });
    ...
    public async Task<ShikimoriAnime?> GetAnime(string id, bool getRelated = false)
    {
        var request = new GraphqlRequest
        {
            ...
            Variables = new Dictionary<string, object>() { { "id", id }, { "getRelated", getRelated } },
        };
    ...
    public async Task<bool> UpdateAnime(
        string id,
        ShikimoriUserRate.StatusEnum updateStatus,
        int progress,
        int? numberOfTimesRewatched = null
    )
    {
        UrlBuilder url = new UrlBuilder { Base = $"{_apiBaseUrl}/v2/user_rates" };

        string? shikimoriUserId = await GetUserId();
        if (shikimoriUserId == null)
            return false;
        ...
        var stringContent = new StringContent(
            JsonSerializer.Serialize(updateBody, jsonSerializerOptions),
            Encoding.UTF8,
            "application/json"
        );
        var apiCall = await _authApiCall.AuthenticatedApiCall(
    ...
    {
        string? shikimoriUserId = _userConfig
            ?.KeyPairs.FirstOrDefault(keypair => keypair.Key == "ShikimoriUserId")
            ?.Value;
    ...
    private async Task<T?> GraphqlApiCall<T>(GraphqlRequest request)
        where T : class
    {
        var url = new UrlBuilder { Base = $"{_apiBaseUrl}/graphql" };
