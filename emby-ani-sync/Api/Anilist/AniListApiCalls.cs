...
namespace emby_ani_sync.Api.Anilist
{
    public class AniListApiCalls
    {
        ...
        ";
            ...
            AniListSearch.AniListSearchMedia result =
                await GraphQlHelper.DeserializeRequest<AniListSearch.AniListSearchMedia>(_httpClient, query, variables);
            ...
            if (result != null)
            {
                if (result.Data.Page.PageInfo.HasNextPage)
                {
                    ...
                    while (page < 10)
                    {
                        ...
                        AniListSearch.AniListSearchMedia nextPageResult =
                            await GraphQlHelper.DeserializeRequest<AniListSearch.AniListSearchMedia>(
                                _httpClient,
                                query,
                                variables
                            );
        ...
        public async Task<AniListSearch.Media> GetAnime(int id)
        {
          ...
          Media(id: $id) {
            ...
            Dictionary<string, object> variables = new Dictionary<string, object> { { "id", id.ToString() } };
            ...
            if (response != null)
            {
                ...
                var result = JsonSerializer.Deserialize<AniListGet.AniListGetMedia>(
        ...
        public async Task<AniListViewer.Viewer> GetCurrentUser()
        {
          ...
          Viewer {
            ...
            {
                StreamReader streamReader = new StreamReader(await response.Content.ReadAsStreamAsync());
                var result = JsonSerializer.Deserialize<AniListViewer.AniListGetViewer>(
                    await streamReader.ReadToEndAsync()
                );
        ...
        public async Task<bool> UpdateAnime(
            int id,
            AniListSearch.MediaListStatus status,
            int progress,
        ...
        public async Task<List<AniListMediaList.Entries>> GetAnimeList(int userId, AniListSearch.MediaListStatus status)
        {
            ...
            AniListMediaList.AniListUserMediaList result =
                await GraphQlHelper.DeserializeRequest<AniListMediaList.AniListUserMediaList>(
                    _httpClient,
                    query,
                    variables
                );
            ...
            if (result?.Data?.MediaListCollection?.MediaList != null)
            {
                ...
                if (result.Data.MediaListCollection.HasNextChunk)
                {
                    ...
                    while (page < 10)
                    {
                        ...
                        AniListMediaList.AniListUserMediaList nextPageResult =
                            await GraphQlHelper.DeserializeRequest<AniListMediaList.AniListUserMediaList>(
                                _httpClient,
                                query,
                                variables
                        ...
                        entriesList = entriesList.Concat(
                            nextPageResult.Data.MediaListCollection.MediaList.SelectMany(entries => entries.Entries)
                        );
