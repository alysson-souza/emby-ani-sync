...
namespace emby_ani_sync.Api.Simkl;
...
public class SimklApiCalls
{
    ...
    {
        UrlBuilder url = new UrlBuilder { Base = $"{ApiBaseUrl}/sync/activities" };

        HttpResponseMessage? apiCall = await _authApiCall.AuthenticatedApiCall(
            ApiName.Simkl,
            AuthApiCall.CallType.GET,
            url.Build(),
    ...
    {
        UrlBuilder url = new UrlBuilder { Base = $"{ApiBaseUrl}/search/anime" };
        ...
        while (page < 10)
        {
            ...
            {
                if (
                    int.TryParse(paginationLimitResults.First(), out int parsedPaginationLimit)
                    && parsedPaginationLimit != pageLimit
                )
                {
            ...
            HttpResponseMessage? pageApiCall = await _authApiCall.AuthenticatedApiCall(
                ApiName.Simkl,
                AuthApiCall.CallType.GET,
                url.Build()
            );
    ...
    public async Task<SimklIdLookupMedia?> GetAnimeByIdLookup(
        AnimeOfflineDatabaseHelpers.OfflineDatabaseResponse ids,
        string title
    )
    {
        UrlBuilder url = new UrlBuilder { Base = $"{ApiBaseUrl}/search/id" };
        ...
        {
            StreamReader streamReader = new StreamReader(await apiCall.Content.ReadAsStreamAsync());
            List<SimklIdLookupMedia>? results = JsonSerializer.Deserialize<List<SimklIdLookupMedia>?>(
                await streamReader.ReadToEndAsync()
            );
            if (results != null && results.Count > 0)
            {
                // attempt to match to title
                var detectedAnime = results.FirstOrDefault(anime =>
                    string.Equals(anime.Title, title, StringComparison.CurrentCultureIgnoreCase)
                );
    ...
    {
        UrlBuilder url = new UrlBuilder { Base = $"{ApiBaseUrl}/sync/all-items/anime" };
        if (status != null)
        {
            url.Base += $"/{status}";
        }
        ...
        {
            StreamReader streamReader = new StreamReader(await apiCall.Content.ReadAsStreamAsync());
            SimklUserList? deserialized = JsonSerializer.Deserialize<SimklUserList>(
                await streamReader.ReadToEndAsync()
            );
    ...
    public async Task<bool> UpdateAnime(
        int animeId,
        SimklStatus updateStatus,
        bool isShow,
        AnimeOfflineDatabaseHelpers.OfflineDatabaseResponse ids,
        int numberOfWatchedEpisodes
    )
    {
        UrlBuilder url = new UrlBuilder { Base = $"{ApiBaseUrl}/sync/history" };
        ...
        SimklExtendedIds convertedIds = new SimklExtendedIds { Simkl = animeId };
        ...
        if (isShow)
        {
            var updateBodyShow = new UpdateBodyShow { Ids = convertedIds };
            ...
            if (simklUpdateAll)
            {
                ...
                {
                    updateBodyShow.Episodes.Add(new UpdateEpisode { EpisodeNumber = i });
                }
            }
            else
            {
                updateBodyShow.Episodes.Add(new UpdateEpisode { EpisodeNumber = numberOfWatchedEpisodes });
            }

            updateBody.Shows = new List<UpdateBodyShow> { updateBodyShow };
        }
        else
        {
            updateBody.Movies = new List<UpdateBodyShow> { new UpdateBodyShow { Ids = convertedIds } };
        }

        var stringContent = new StringContent(JsonSerializer.Serialize(updateBody), Encoding.UTF8, "application/json");

        HttpResponseMessage? response = await _authApiCall.AuthenticatedApiCall(
            ApiName.Simkl,
            AuthApiCall.CallType.POST,
            url.Build(),
