...
namespace emby_ani_sync.Api
{
    public class ApiAuthentication
    {
        ...
        private readonly string _codeChallenge =
            "eZBLUX_JPk4~el62z_k3Q4fV5CzCYHoTz4iLKvwJ~9QTsTJNlzwveKCSYCSiSOa5zAm5Zt~cfyVM~3BuO4kQ0iYwCxPoeN0SOmBYR_C.QgnzyYE4KY-xIe4Vy1bf7_B4";
        ...
        public ApiAuthentication(
            ...
            ProviderApiAuth? overrideProviderApiAuth = null,
            string? overrideRedirectUrl = null
        )
        {
            _provider = provider;
            ...
            {
                _redirectUrl = overrideRedirectUrl.TrimEnd('/') + "/emby/AniSync/authCallback";
            }
            else if (userCallbackUrl != null)
            {
                _redirectUrl = userCallbackUrl.TrimEnd('/') + "/emby/AniSync/authCallback";
            }
            else
            {
                var scheme = serverApplicationHost.EnableHttps ? "https" : "http";
                var port = serverApplicationHost.EnableHttps
                    ? serverApplicationHost.HttpsPort
                    : serverApplicationHost.HttpPort;
                _redirectUrl = $"{scheme}://localhost:{port}/emby/AniSync/authCallback";
            }
        }
        ...
        public async Task<UserApiAuth> GetToken(Guid userId, string? code = null, string? refreshToken = null)
        {
            ...
            if (_provider == ApiName.Kitsu)
            {
                ...
                {
                    formUrlEncodedContent = new FormUrlEncodedContent(
                        new[]
                        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        formUrlEncodedContent = new StringContent(
                            JsonSerializer.Serialize(content.ToDictionary(item => item.Key, item => item.Value))
                        );
                    }
                    else
                    {
            ...
            var response = client
                .PostAsync(
                    new Uri(
                        $"{(_provider is not ApiName.Simkl ? _authApiUrl : $"{SimklApiCalls.ApiBaseUrl}/oauth")}/token"
                    ),
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                UserConfig? pluginConfig = Plugin.Instance.PluginConfiguration.UserConfig.FirstOrDefault(item =>
                    item.UserId == userId
                );
                ...
                throw new NullReferenceException(
                    "The user you are attempting to authenticate does not exist in the plugins config file"
                );
            }

            throw new AuthenticationException(
                $"Could not retrieve {_provider} token: "
                    + response.StatusCode
                    + " - "
