...
namespace emby_ani_sync.Api.Kitsu
{
    public class KitsuApiCalls
    {
        ...
        {
            UrlBuilder url = new UrlBuilder { Base = $"{ApiUrl}/anime" };
            ...
            if (apiCall != null)
            {
                ...
                var animeList = JsonSerializer.Deserialize<KitsuSearch.KitsuSearchMedia>(
        ...
        public async Task<MalApiCalls.User> GetUserInformation()
        {
            ...
            if (apiCall != null)
            {
                ...
                StreamReader streamReader = new StreamReader(await apiCall.Content.ReadAsStreamAsync());
                var animeList = JsonSerializer.Deserialize<KitsuGetUser.KitsuUserRoot>(
                    await streamReader.ReadToEndAsync()
                );
                ...
                {
                    Id = animeList.KitsuUserList[0].Id,
                    Name = animeList.KitsuUserList[0].KitsuUser.Name,
                };
            }
        ...
        {
            UrlBuilder url = new UrlBuilder { Base = $"{ApiUrl}/anime/{animeId}" };
        ...
        {
            UrlBuilder url = new UrlBuilder { Base = $"{ApiUrl}/anime/{animeId}/media-relationships" };
            ...
            try
            {
                ...
                {
                    StreamReader streamReader = new StreamReader(await apiCall.Content.ReadAsStreamAsync());
                    var mediaRelationships = JsonSerializer.Deserialize<KitsuMediaRelationship.MediaRelationship>(
                        await streamReader.ReadToEndAsync()
                    );
                    ...
                    if (mediaRelationships != null)
                    {
                        foreach (KitsuMediaRelationship.RelationshipData relationshipData in mediaRelationships.Data)
                        {
                            ...
                            {
                                listOfRelatedAnimeIds.Add(
                                    int.Parse(relationshipData.Relationships.Destination.RelationshipData.Id)
                                );
                            }
                        }
                        ...
                        {
                            mediaRelationships.Included[i].RelationType = mediaRelationships
                                .Data[i]
                                .Attributes
                                .RelationType;
                        }
                        ...
                        return mediaRelationships
                            .Included.Where(anime => listOfRelatedAnimeIds.Any(ids => anime.Id == ids))
                            .ToList();
                    }
                }
            }
        ...
        public async Task<bool> UpdateAnimeStatus(
            int animeId,
            int numberOfWatchedEpisodes,
            KitsuUpdate.Status status,
        ...
        )
        {
            ...
            if (userId != null)
            {
                ...
                var payload = new KitsuUpdate.KitsuLibraryEntryPostPatchRoot
                {
                    ...
                    {
                        Type = "libraryEntries",
                        Attributes = new KitsuUpdate.Attributes { Status = status, Progress = numberOfWatchedEpisodes },
                        Relationships = new KitsuUpdate.Relationships
                        {
                            AnimeData = new KitsuUpdate.AnimeData
                            {
                                Anime = new KitsuSearch.KitsuAnime { Type = "anime", Id = animeId },
                            },
                            UserData = new KitsuUpdate.UserData
                            {
                                User = new KitsuGetUser.KitsuUser { Type = "users", Id = userId.Value },
                            },
                        },
                    },
                };
                ...
                HttpResponseMessage? apiCall = await _authApiCall.AuthenticatedApiCall(
                    ApiName.Kitsu,
                    libraryStatus != null ? AuthApiCall.CallType.PATCH : AuthApiCall.CallType.POST,
                    url.Build(),
        ...
        private async Task<KitsuUpdate.KitsuLibraryEntryListRoot> LibraryEntriesCall(
            List<KeyValuePair<string, string>> parameters
        )
        {
            UrlBuilder url = new UrlBuilder { Base = $"{ApiUrl}/library-entries", Parameters = parameters };
            ...
            try
            {
                ...
                {
                    StreamReader streamReader = new StreamReader(await apiCall.Content.ReadAsStreamAsync());
                    var library = JsonSerializer.Deserialize<KitsuUpdate.KitsuLibraryEntryListRoot>(
                        await streamReader.ReadToEndAsync()
                    );
