...
namespace emby_ani_sync.Api.Annict
{
    public class AnnictApiCalls
    {
        ...
        ";
            ...
            Dictionary<string, object> variables = new Dictionary<string, object> { { "title", searchString } };

            AnnictSearch.AnnictSearchMedia searchMedia = new AnnictSearch.AnnictSearchMedia();

            var response = await GraphQlHelper.AuthenticatedRequest(
                _httpClientFactory,
                _loggerFactory,
            ...
            if (response != null)
            {
                ...
                var result = JsonSerializer.Deserialize<AnnictSearch.AnnictSearchMedia>(
        ...
        ";
            ...
            {
                StreamReader streamReader = new StreamReader(await response.Content.ReadAsStreamAsync());
                result = JsonSerializer.Deserialize<AnnictMediaList.AnnictUserMediaList>(
                    await streamReader.ReadToEndAsync()
                );
            }
            if (result != null)
            {
                while (result.AnnictSearchData.Viewer.AnnictUserMediaListLibraryEntries.PageInfo.HasNextPage)
                {
                    variables["after"] = result
                        .AnnictSearchData
                        .Viewer
                        .AnnictUserMediaListLibraryEntries
                        .PageInfo
                        .StartCursor;
                    var paginatedResponse = await GraphQlHelper.AuthenticatedRequest(
                        _httpClientFactory,
                        _loggerFactory,
                        _serverApplicationHost,
                        _memoryCache,
                    ...
                    if (paginatedResponse != null)
                    {
                        StreamReader streamReader = new StreamReader(
                            await paginatedResponse.Content.ReadAsStreamAsync()
                        );
                        paginatedResult = JsonSerializer.Deserialize<AnnictMediaList.AnnictUserMediaList>(
                            await streamReader.ReadToEndAsync()
                        );
                    }
                    result.AnnictSearchData.Viewer.AnnictUserMediaListLibraryEntries.Nodes = result
                        .AnnictSearchData.Viewer.AnnictUserMediaListLibraryEntries.Nodes.Concat(
                            paginatedResult.AnnictSearchData.Viewer.AnnictUserMediaListLibraryEntries.Nodes
                        )
                        .ToList();
                    result.AnnictSearchData.Viewer.AnnictUserMediaListLibraryEntries.PageInfo = paginatedResult
                        .AnnictSearchData
                        .Viewer
                        .AnnictUserMediaListLibraryEntries
                        .PageInfo;
        ...
        public async Task<AnnictSearch.AnnictAnime> GetAnime(string id)
        {
          ...
          node(id: $id) {
            ...
            Dictionary<string, object> variables = new Dictionary<string, object> { { "id", id } };
            ...
            {
                StreamReader streamReader = new StreamReader(await response.Content.ReadAsStreamAsync());
                var result = JsonSerializer.Deserialize<AnnictGetMedia.AnnictGetMediaRoot>(
                    await streamReader.ReadToEndAsync()
                );
        ...
        public async Task<AnnictViewer.AnnictViewerRoot> GetCurrentUser()
        {
          ...
          viewer {
            ...
            {
                StreamReader streamReader = new StreamReader(await response.Content.ReadAsStreamAsync());
                var result = JsonSerializer.Deserialize<AnnictViewer.AnnictViewerRoot>(
                    await streamReader.ReadToEndAsync()
                );
