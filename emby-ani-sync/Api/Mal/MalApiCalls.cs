...
namespace emby_ani_sync.Api
{
    public class MalApiCalls
    {
        ...
        {
            UrlBuilder url = new UrlBuilder { Base = $"{ApiUrl}/users/@me" };
        ...
        public async Task<List<UserAnimeListData>> GetUserAnimeList(
            Status? status = null,
            Sort? sort = null,
            int? idSearch = null
        )
        {
            UrlBuilder url = new UrlBuilder { Base = $"{ApiUrl}/users/@me/animelist" };
            ...
            while (builtUrl != null)
            {
                ...
                if (apiCall != null)
                {
                    ...
                    var options = new JsonSerializerOptions();
                    options.Converters.Add(new JsonStringEnumConverter());
                    UserAnimeList userAnimeListPage = JsonSerializer.Deserialize<UserAnimeList>(
                    ...
                    if (userAnimeListPage?.Data != null && userAnimeListPage.Data.Count > 0)
                    {
                        ...
                        if (userAnimeListPage.Paging.Next != null)
                        {
                            ...
                            _logger.LogInformation(
                                $"Additional pages found; waiting 2 seconds before calling again..."
                            );
                            Thread.Sleep(2000);
                        }
                        else
        ...
        public async Task<UpdateAnimeStatusResponse> UpdateAnimeStatus(
            int animeId,
        ...
        {
            UrlBuilder url = new UrlBuilder { Base = $"{ApiUrl}/anime/{animeId}/my_list_status" };
            ...
            {
                body.Add(
                    new KeyValuePair<string, string>("num_times_rewatched", numberOfTimesRewatched.Value.ToString())
                );
            }
            ...
            try
            {
                var apiCall = await _authApiCall.AuthenticatedApiCall(
                    ApiName.Mal,
                    AuthApiCall.CallType.PUT,
                    builtUrl,
                    new FormUrlEncodedContent(body.ToArray())
                );
                ...
                if (apiCall != null)
                {
                    ...
                    updateResponse = JsonSerializer.Deserialize<UpdateAnimeStatusResponse>(
                        await streamReader.ReadToEndAsync(),
                        options
                    );
