...
public class Helpers
{
    ...
    public static void MockHttpCalls(List<HttpCall> calls, ref IHttpClientFactory _httpClientFactory)
    {
        ...
        var clientHandlerStub = new DelegatingHandlerStub(
            ...
            {
                foreach (
                    HttpCall httpCall in calls
                        .OrderByDescending(call => call.RequestMethod != null)
                        .ThenByDescending(call => call.RequestUrlMatch != null)
                )
                {
                    ...
                    if (
                        httpCall.RequestUrlMatch != null
                        && request.RequestUri != null
                        && !httpCall.RequestUrlMatch(request.RequestUri.GetLeftPart(UriPartial.Path).ToString())
                    )
                    {
                        continue;
                    }
                    ...
                    return Task.FromResult(
                        new HttpResponseMessage(httpCall.ResponseCode)
                        {
                            RequestMessage = request,
                            Content = new StringContent(httpCall.ResponseContent),
                        }
                    );
                }
