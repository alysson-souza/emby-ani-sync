...
public class Shikimori
{
    ...
    private void Setup(List<Helpers.HttpCall> httpCalls)
    {
        ...
        _shikimoriApiCalls = new ShikimoriApiCalls(
            _httpClientFactory,
            _loggerFactory,
            _serverApplicationHost.Object,
            memoryCache,
            mockDelayer.Object,
            new Dictionary<string, string> { { "User-Agent", "awd" } },
            new UserConfig
            {
                UserApiAuth = new[]
                {
        ...
        Setup(
            new List<Helpers.HttpCall>
            {
                new()
                {
                    ...
                    ResponseContent = JsonSerializer.Serialize(new ShikimoriApiCalls.User { Id = 1 }),
                },
            }
        );
            ...
            new List<Helpers.HttpCall>
            {
                new()
                {
                    ...
                    ResponseContent = JsonSerializer.Serialize(
                        new ShikimoriApiCalls.GraphqlResponse<Dictionary<string, List<ShikimoriAnime>>>
                        {
                            Data = new Dictionary<string, List<ShikimoriAnime>>
                            {
                                {
                                    "animes",
                                    new List<ShikimoriAnime> { new() { Id = 1.ToString() } }
                                },
                            },
                        }
                    ),
    ...
    public async Task TestGenericSearchRetrievingMaximumResults()
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            shikimoriMediaList.Add(new ShikimoriAnime { Id = i.ToString() });
        }
            ...
            new List<Helpers.HttpCall>
            {
                new()
                {
                    ...
                    ResponseContent = JsonSerializer.Serialize(
                        new ShikimoriApiCalls.GraphqlResponse<Dictionary<string, List<ShikimoriAnime>>>
                        {
                            Data = new Dictionary<string, List<ShikimoriAnime>> { { "animes", shikimoriMediaList } },
                        }
                    ),
                },
            }
        ...
        Setup(
            new List<Helpers.HttpCall>
            {
                ...
                new()
                {
                    ...
                    ResponseCode = HttpStatusCode.OK,
                    ResponseContent = JsonSerializer.Serialize(
                        new List<ShikimoriUpdate.UserRate> { new() { AnimeId = 1.ToString() } }
                    ),
                },
                new()
                {
