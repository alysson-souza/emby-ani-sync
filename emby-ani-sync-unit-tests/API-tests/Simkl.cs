...
namespace emby_ani_sync_unit_tests.API_tests;
...
public class Simkl
{
    ...
    public void Setup(List<Helpers.HttpCall> httpCalls)
    {
        ...
        _simklApiCalls = new SimklApiCalls(
            _httpClientFactory,
            mockLoggerFactory,
            mockServerApplicationHost.Object,
            memoryCache,
            mockDelayer.Object,
            new Dictionary<string, string> { { "simkl-api-key", String.Empty } },
            new UserConfig
            {
                UserApiAuth = new[]
                {
                    new UserApiAuth { Name = ApiName.Simkl, AccessToken = String.Empty },
                },
            }
        );
    }
        ...
        Setup(
            new List<Helpers.HttpCall>
            {
                new()
                {
                    ...
                    ResponseCode = HttpStatusCode.OK,
                    ResponseContent = JsonSerializer.Serialize(new List<SimklMedia> { new() { Title = String.Empty } }),
                },
            }
        );
        ...
        Setup(
            new List<Helpers.HttpCall>
            {
                new()
                {
                    ...
                    ResponseCode = HttpStatusCode.OK,
                    ResponseContent = JsonSerializer.Serialize(new SimklExtendedMedia { Title = String.Empty }),
                },
            }
        );
            ...
            new List<Helpers.HttpCall>
            {
                new()
                {
                    ...
                    ResponseContent = JsonSerializer.Serialize(
                        new SimklUserList { Entry = new List<SimklUserEntry> { new() { } } }
                    ),
                },
            }
        );
        ...
        Setup(
            new List<Helpers.HttpCall>
            {
                new()
                {
                    ...
                    ResponseCode = HttpStatusCode.OK,
                    ResponseContent = JsonSerializer.Serialize(
                        new List<SimklIdLookupMedia> { new() { Title = title } }
                    ),
                },
            }
        );

        var result = await _simklApiCalls.GetAnimeByIdLookup(
            new AnimeOfflineDatabaseHelpers.OfflineDatabaseResponse { MyAnimeList = id },
            title
        );
    ...
    public async Task TestUpdateAnime()
    {
        ...
        var result = await _simklApiCalls.UpdateAnime(
            45006,
            SimklStatus.plantowatch,
            true,
            new AnimeOfflineDatabaseHelpers.OfflineDatabaseResponse { AniDb = 6327 },
            10
        );
