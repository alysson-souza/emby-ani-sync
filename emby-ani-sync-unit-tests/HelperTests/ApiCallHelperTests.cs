...
public class ApiCallHelperTests
{
    ...
    public void AniListSearchAnimeConvertedListTest()
    {
        ...
        for (int i = 0; i < animeList.Count; i++)
        {
            ...
            Assert.AreEqual(
                new List<string>
                {
                    mediaList[i].Title.Romaji,
                    mediaList[i].Title.UserPreferred,
                    "Synonym1",
                    "Synonym2",
                },
                animeList[i].AlternativeTitles.Synonyms
            );
        }
    }
    ...
    public void KitsuSearchAnimeConvertedListTest()
    {
        ...
        for (int i = 0; i < animeList.Count; i++)
        {
            ...
            Assert.AreEqual(
                new List<string>
                {
                    mediaList[i].Attributes.Slug,
                    mediaList[i].Attributes.CanonicalTitle,
                    "Synonym1",
                    "Synonym2",
                },
                animeList[i].AlternativeTitles.Synonyms
    ...
    public void ShikimoriSearchAnimeConvertedListTest()
    {
        ...
        for (int i = 0; i < animeList.Count; i++)
        {
            ...
            Assert.AreEqual(
                mediaList[i].Synonyms.Append(mediaList[i].Russian).ToList(),
                animeList[i].AlternativeTitles.Synonyms
            );
            ...
            Assert.AreEqual(
                mediaList[i].UserRate.Status == ShikimoriUserRate.StatusEnum.rewatching,
                animeList[i].MyListStatus.IsRewatching
            );
    ...
    public void KitsuConfirmRelatedAnimeExists()
    {
        ...
        foreach (var anime in animeList)
        {
            ...
            {
                var matchingRelated = mediaList
                    .FirstOrDefault(item => item.Id == anime.Id)
                    ?.RelatedAnime.FirstOrDefault(item => item.Id == relatedAnime.Anime.Id);
                if (matchingRelated == null)
                    continue;
                ...
                Assert.AreEqual(
                    new List<string>
                    {
                        matchingRelated.Attributes.Slug,
                        matchingRelated.Attributes.CanonicalTitle,
                        "Synonym1",
                        "Synonym2",
                    },
                    relatedAnime.Anime.AlternativeTitles.Synonyms
                );
            }
        }
    }
    ...
    public async Task KitsuGetConvertedUserListTest(
        int progress,
        bool reconsuming,
        int reconsumeCount,
        KitsuUpdate.Status status,
        int expectedProgress,
        bool expectedReconsuming,
        int expectedReconsumeCount,
    ...
    )
    {
        ...
        int animeId = 1;
        Helpers.MockHttpCalls(
            new List<Helpers.HttpCall>
            {
                new()
                {
                    ...
                    ResponseContent = JsonSerializer.Serialize(
                        new KitsuUpdate.KitsuLibraryEntryListRoot
                        {
    ...
    private AniListSearch.Media GetAniListMedia(bool createRelations)
    {
        ...
        if (createRelations)
        {
            ...
            for (int i = 0; i < random.Next(0, 5); i++)
            {
                mediaConnection.Media.Add(
                    new AniListSearch.MediaEdge
                    {
